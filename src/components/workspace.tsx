...
function DashboardContent() {
  return <>
    <section className="workspace-stat-grid" aria-label="Workspace overview">
      {["Tools", "Checked out", "Needs attention", "Recent activity"].map((label) => <article className="workspace-stat" key={label}><span>{label}</span><strong>—</strong><small>Awaiting workspace data</small></article>)}
    </section>
...
function TableContent({ view }: { view: WorkspaceView }) {
  return <section className="workspace-panel workspace-table-panel"><div className="workspace-panel-heading"><div><p className="workspace-eyebrow">{view.eyebrow}</p><h2>{view.title}</h2></div><div className="workspace-heading-actions"><ActionLink action={view.secondaryAction} quiet /><ActionLink action={view.primaryAction} /></div></div><div className="workspace-table-wrap"><table><thead><tr>{(view.columns ?? []).map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody><tr><td colSpan={view.columns?.length ?? 1}><WorkspaceEmptyState view={view} /></td></tr></tbody></table></div></section>;
}
