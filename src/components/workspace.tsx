...
function TableContent({ view, tools, locations }: { view: WorkspaceView; tools: ToolSummary[] | null; locations: LocationSummary[] | null }) {
  return <section className="workspace-panel workspace-table-panel"><div className="workspace-panel-heading"><div><p className="workspace-eyebrow">{view.eyebrow}</p><h2>{view.title}</h2></div></div><div className="workspace-table-wrap"><table><thead><tr>{(view.columns ?? []).map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{view.key === "tools" && tools?.length ? tools.map((tool) => <tr key={tool.id}><td>{tool.name}</td><td>{tool.asset_code}</td><td>{tool.status.replaceAll("_", " ")}</td><td>{tool.updated_at.slice(0, 10)}</td></tr>) : view.key === "locations" && locations?.length ? locations.map((location) => <tr key={location.id}><td>{location.name}</td><td>{location.type.replaceAll("_", " ")}</td><td>{location.address || "—"}</td><td>{location.active ? "Active" : "Inactive"}</td><td>{location.updated_at.slice(0, 10)}</td></tr>) : <tr><td colSpan={view.columns?.length ?? 1}><WorkspaceEmptyState view={view} /></td></tr>}</tbody></table></div></section>;
}
