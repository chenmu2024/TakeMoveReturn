...
export default async function ContentPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const eyebrow = seoPage?.pageType === "blog" || seoPage?.pageType === "guide"
    ? "PRACTICAL GUIDE"
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  ...
  return {
    ...
    robots: seoPage && (seoPage.status !== "indexable" || seoPage.needsUSVerification) ? { index: false, follow: false } : { index: true, follow: true },
  };
}
